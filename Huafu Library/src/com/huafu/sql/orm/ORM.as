...
package com.huafu.sql.orm
{
	 ...
	 * 		[HasMany(foreignColumn="user_id", foreignClass="UserTag")]
	 ...
	 * 		[HasMany(foreignClass="Tag", usingClass="UserTag")]
	...
	public dynamic class ORM extends ObjectProxy
	{
		...
		{
			var res : Array, ev : ORMEvent, sql : String, params : Object = {}, q : SQLiteQuery = getPreparedQuery();
		 ...
		 * </listing>
		 */
		public function findAll( params : Object = null, orderBy : Object = null, limit : int = -1, offset : int
								 = 1 ) : ORMIterator
		{
			var q : SQLiteQuery = getPreparedQuery(), value : *, name : String, prop : ORMPropertyDescriptor,
			...
			if (orderBy)
			{
				...
				{
					prop = ormDescriptor.propertyDescriptor(name);
					q.orderBy(prop.columnName + " " + (!orderBy[name] || String(orderBy[name]).toUpperCase()
							  == "DESC" ? "DESC" : "ASC"));
				}
			}
			if (limit > 0)
		 ...
		 */
		public function findAllBySql( whereSql : String, params : Object = null, orderBySql : String
									  = null, groupBySql : String = null, limit : int = -1, offset : int
									  = 1 ) : ORMIterator
		{
		 ...
		 */
		public function getPreparedQuery( tableAlias : String = null, setDeletedCondition : Boolean
										  = true ) : SQLiteQuery
		{
		...
		public function remove() : Boolean
		{
			...
			stmt = connection.createStatement("/* " + PREPEND_SQL_COMMENT + " */ DELETE FROM " + ormDescriptor.
											  tableName + " WHERE " + primaryKeyColumnName + " = :" + primaryKeyPropertyName);
		...
		{
			var ev : ORMEvent = new ORMEvent(ORMEvent.SAVING), sql : String, res : SQLResult, parts : Array,
				params : Object = {}, name : String, stmt : SQLiteStatement, prop : ORMPropertyDescriptor,
				rel : ORMRelationHasOne;
			...
			if (primaryKeyValue)
			{
				...
				sql += parts.join(", ") + " WHERE " + primaryKeyColumnName + " = :" + primaryKeyPropertyName;
			...
			else
			{
				...
				_hasChanged.push(ormDescriptor.createdAtProperty.name, ormDescriptor.updatedAtProperty.
								 name);
		...
		private function _propertyChangeHandler( event : PropertyChangeEvent ) : void
		{
			...
			var pName : String = event.property.toString(), rel : IORMRelation, prop : ORMPropertyDescriptor
				= _descriptor.propertyDescriptor(pName);
			if (!prop)
			{
				if (!(rel = _descriptor.getRelatedTo(pName)))
				{
					throw new IllegalOperationError("Trying to access a unknown property '" + pName + "'. Define it first in the model if you wish to use it in your code if that is a column of the related table.");
				}
			}
			// here it's a property of the ORM that has changed
			if (prop && prop.isReadOnly)
			{
				throw new IllegalOperationError("The property '" + prop.name + "'  of model '" + _descriptor.
												ormClassQName + "' is read-only");
			}
